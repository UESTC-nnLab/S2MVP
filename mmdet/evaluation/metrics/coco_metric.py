        ...
        for metric in self.metrics:
            ...
            if metric == 'proposal':
                ...
                for metric_item in metric_items:
                 ...
                 #######将yolox的mAP计算结果输出到日志中
                pres = coco_eval.eval['precision']
                pre_50 = pres[0,:,0,0,-1]  # 第三为类别 (T,R,K,A,M)
                recs = coco_eval.eval['recall']
                rec_50 = recs[0,0,0,-1] # 第二为类别 (T,K,A,M)
                pre = np.mean(pre_50[:int(rec_50*100)])
                F1 = 2*pre*rec_50/(pre+rec_50)
                logger.info(" Precision: %.4f, Recall: %.4f, F1: %.4f" %(pre, rec_50, F1))
                logger.info("Get map done.")
                #######PR cureve save
                with open("PG_DAUB.txt", 'w') as f:
                    for pred in pre_50:
