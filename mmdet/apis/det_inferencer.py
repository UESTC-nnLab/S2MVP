...
from mmcv.transforms import LoadImageFromFile, LoadImagesFromFile
...
def register_hook(model, func, hook_layers):
            ...
            for name, layer in model.named_modules():
                # print(f"name: {name}")
                # if name in hook_layers: ###注释掉这个，表示所有层都注册hook
...
class DetInferencer(BaseInferencer):
    ...
    def _init_pipeline(self, cfg: ConfigType) -> Compose:
        ...
        load_img_idx = self._get_transform_idx(
            pipeline_cfg, ('LoadImagesFromFile', LoadImagesFromFile))
        if load_img_idx == -1:
            raise ValueError(
    ...
    def __call__(
        ...
        for ori_imgs, data in (track(inputs, description='Inference')
            ...
            ################feature_map
            hook_layers = ['teacher.backbone.layer1.0','teacher.backbone.layer2.0', 'teacher.backbone.layer3.0', 'teacher.backbone.layer4.0', 'teacher.neck.lateral_convs.0', 'teacher.neck.fpn_convs.0', 'teacher.neck.fpn_convs.1', 'teacher.neck.fpn_convs.2', 'teacher.neck.fpn_convs.3', 'teacher.neck.fpn_convs.4',
                           'teacher.neck.fpn.convgru_forward','teacher.neck.fpn.convgru_backward','teacher.neck.fpn.bidirection_conv',
                           'teacher.neck.fpn.conv_ref','teacher.neck.fpn.conv_cur','teacher.neck.fpn.conv_cr_mix','teacher.neck.fpn.align','teacher.enhance']
            ...
            for fea in layer_outputs:
                i = i+1
                #判断fea是不是tensor
                if not isinstance(fea, torch.Tensor):
                    continue
                #判断fea不是4维度
                if len(fea.shape) != 4:
                    continue
                ...
                cv2.imwrite('{}.jpg'.format(i), heatmap) #hook_layers[i-1]
            ##########################
            ####################可视化每个通道的特征图
            # for i, features in enumerate(layer_outputs):  # 遍历每层输出
            #     # features 的形状一般为 torch.Size([batch_size, channels, height, width])
            #     features = features[0]  # 选择第一个样本（batch size为1时）
                
            #     num_channels = features.size(0)  # 获取通道数
                
            #     # 可视化每个通道的特征图
            #     for j in range(num_channels):
            #         feature_map = features[j].cpu().detach().numpy()  # 取出第 j 个通道的特征图
            #         feature_map = (feature_map - feature_map.min()) / (feature_map.max() - feature_map.min() + 1e-5) * 255  # 归一化到0-255
            #         feature_map = feature_map.astype(np.uint8)  # 转为uint8类型
                    
            #         src_size = (640, 480)  # 原图尺寸
            #         feature_map = cv2.resize(feature_map, src_size, interpolation=cv2.INTER_LINEAR)  # 调整尺寸
                    
            #         # 将灰度特征图转为3通道图像方便保存和显示
            #         feature_map_colored = cv2.applyColorMap(feature_map, cv2.COLORMAP_JET)
                    
            #         cv2.imwrite('layer_{}_channel_{}.jpg'.format(i, j), feature_map_colored)  # 保存每个通道的特征图
            ########可视化单个通道的特征图
            # channel_to_visualize = 0  # 选择要展示的通道索引
            # for i, features in enumerate(layer_outputs):  # 遍历每层输出
            #     # features 的形状一般为 torch.Size([batch_size, channels, height, width])
            #     features = features[0]  # 选择第一个样本（batch size为1时）
                
            #     # 只展示一个通道，选择通道 channel_to_visualize
            #     feature_map = features[channel_to_visualize].cpu().detach().numpy()  # 取出指定通道的特征图
            #     feature_map = (feature_map - feature_map.min()) / (feature_map.max() - feature_map.min() + 1e-5) * 255  # 归一化到0-255
            #     feature_map = feature_map.astype(np.uint8)  # 转为uint8类型
            ...
            #     cv2.imwrite('layer_{}_channel_{}.jpg'.format(i, channel_to_visualize), feature_map_colored)  # 保存指定通道的特征图
